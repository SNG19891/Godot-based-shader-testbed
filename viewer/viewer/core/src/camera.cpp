...
void Camera::setRotation(QMatrix4x4 & m)
{
    // qDebug() << "setRotation: \n" << pangleX << ", " << pangleY << ", " << pangleZ;
    // Extract Euler angles from rotation matrix
    float pitch = m(2, 1);
    if (pitch < 0.9999f && pitch > -0.9999f) {
        
        // 1. Compute Angle X
        pangleX = degrees(qAsin(pitch));
        // 2. Compute Angle Y   
        pangleY = degrees(qAtan2(m(2,0), m(2,2)));
        // 3. Compute Angle Z
        pangleZ = degrees(qAtan2(m(0,1), m(1,1)));
        // qDebug() << "setRotation updated: \n" << pangleX << ", " << pangleY << ", " << pangleZ;
    } else {
        // Gimbal lock case
        pangleZ = 0;
        pangleX = (pitch>0) ? 90.f : -90.f;
        pangleY = degrees(qAtan2(-m(0,2), m(0,0)))  ;
        // qDebug() << "setRotation Gimbal lock: \n" << pangleX << ", " << pangleY << ", " << pangleZ;
    }
}
