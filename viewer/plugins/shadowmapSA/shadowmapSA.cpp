...
{
    cout << "[ShadowmapSA plugin] C - Copy camera position to light position" << endl;
    ...
    vs->compileSourceFile(glwidget()->getPluginPath()+ "/../shadowmapSA/shadowmapSA.vert");
    //cout << "vs" << endl;

    fs = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fs->compileSourceFile(glwidget()->getPluginPath()+"/../shadowmapSA/shadowmapSA.frag");
    //cout << "fs" << endl;
    ...
    g.glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, scale*SHADOW_MAP_WIDTH,
		   scale*SHADOW_MAP_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
...
bool ShadowMap::paintGL()
{
    ...
    GLWidget &g = *glwidget();
    // g.makeCurrent();
    g.resize(SHADOW_MAP_WIDTH,SHADOW_MAP_HEIGHT); // here in case it changed size
    //**
    // In case the window has moved to another display, or the user
    // has changed the scaling:
    if (g.devicePixelRatio() != scale) {
        scale = g.devicePixelRatio();
        g.glActiveTexture(GL_TEXTURE0);
        g.glBindTexture(GL_TEXTURE_2D, textureId);
        g.glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32,
                       scale*SHADOW_MAP_WIDTH, scale*SHADOW_MAP_HEIGHT, 0,
    ...
    glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, scale*SHADOW_MAP_WIDTH,
			scale*SHADOW_MAP_HEIGHT);
