...
void PluginLoader::reloadPlugin()
{
...
#ifdef DEBUG
  cout << "%%% about to load " << filename.toStdString() << endl;
#endif
  // clean-up
  ...
  QDir pluginsDir(glwidget()->getPluginPath());
  QStringList arguments("-f");
#ifndef __APPLE__
#  ifdef DEBUG
  cout << "%%% pluginPath: " << pluginsDir.canonicalPath().toStdString() << endl;
  cout << "%%% Found " << pluginsDir.entryList(QStringList("lib"+filename+"*[0-9]*.so"),
					       QDir::Files|QDir::CaseSensitive, QDir::Unsorted).count()
       << "files" <<endl;

  cout << "%%% about to exec /usr/bin/rm " << (arguments + pluginsDir.entryList(QStringList("lib"+filename+"*[0-9]*.so"),
							     QDir::Files|QDir::CaseSensitive,
							     QDir::Unsorted).replaceInStrings(
								    QRegularExpression("^"), pluginsDir.canonicalPath()+"/")).
                                                                      join("#").toStdString() <<endl;
#  endif
  cout << "rm process exec returns "
       << QProcess::execute(QString("/usr/bin/rm"),
			    arguments + pluginsDir.entryList(QStringList("lib"+filename+"*[0-9]*.so"),
							     QDir::Files|QDir::CaseSensitive,
							     QDir::Unsorted).replaceInStrings(
								    QRegularExpression("^"), pluginsDir.canonicalPath()+"/"))
       << endl;
#else  // NEEDS UPDAT TO PREVIOUS STRATEGY
  cout << "rm process exec returns "
       << QProcess::execute(QString("/usr/bin/rm"),
			    arguments + pluginsDir.entryList(QStringList("lib"+filename+"*[0-9]*.dylib"),
							     QDir::Files|QDir::CaseSensitive,
							     QDir::Unsorted).replaceInStrings(
								    QRegularExpression("^"), pluginsDir.canonicalPath()+"/"))
       << endl;
  ...
  if (not filePRO.open(QFile::WriteOnly | QFile::Truncate)) {
    cout << "### file "<< (filename+"/"+filename+".pro").toStdString()
  ...
  int execRes = QProcess::execute(qmakePath, QStringList());
...
void replace(QString filename, QString old, QString news)
{
  ...
  if (not file.open(QIODevice::ReadWrite)) {
    cout << "### Cannot open file " << filename.toStdString() << endl;
    return;
  }
