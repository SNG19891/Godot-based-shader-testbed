...
{
    if (testMode)
        return; // tests are run in batch mode...
    GLWidget &g = *glwidget();
...
void ShaderLoader::uniformMult(bool up)
{
    ...
    {
        g.glGetUniformiv(program->programId(), id, iparams);
        bool value = iparams[0];
        program->setUniformValue(name, not value);
    }
}
...
void ShaderLoader::showMenu(const QPoint &pos)
{
    ...
    // Info
    vector<QAction *> info;
    info.push_back(new QAction(QString("Current path is ") + QDir::currentPath(), &g));
    if (filename != "")
    {
        // VS
        ...
        if (!enabled_vs)
            aux += QString("[DISABLED]");
        else
        {
            if (vs && vs->isCompiled())
                aux += QString("[COMPILE: OK]");
            else
                aux += QString("[COMPILE: ERROR]");
        }

        info.push_back(new QAction(aux, &g));
        ...
        if (program)
        {
            if (program->isLinked())
            {
                ...
                info.push_back(new QAction(QString("PROGRAM LINK: OK"), &g));
            }
            else
            {
                // g.glColor3f(0.5, 0.0, 0.0);
                info.push_back(new QAction(QString("PROGRAM LINK: ERROR"), &g));
            }
        }
    }
    else
        info.push_back(new QAction(QString("No shaders loaded"), &g));

    // Shaders
    QAction *actionNewVSFS = new QAction("New VS+FS...", &g);
    // actionNew->setShortcut(QString("Ctrl+N"));
    connect(actionNewVSFS, SIGNAL(triggered()), this, SLOT(createDefaultShadersVSFS()));

    QAction *actionNewVSGSFS = new QAction("New VS+GS+FS...", &g);
    // actionNew->setShortcut(QString("Ctrl+N"));
    connect(actionNewVSGSFS, SIGNAL(triggered()), this, SLOT(createDefaultShadersVSGSFS()));

    QAction *actionCreateFromTemplate = new QAction("New (from existing shaders)...", &g);
    connect(actionCreateFromTemplate, SIGNAL(triggered()), this, SLOT(createShaderFromTemplate()));

    QAction* actionNewVSFSInGodot = new QAction("New VS+FS in Godot...", glwidget());
    connect(actionNewVSFSInGodot, SIGNAL(triggered()), this, SLOT(createDefaultShadersVSFSGodot()));

    QAction* actionExportVSFSToGodot = new QAction("Export VS+FS to Godot...", glwidget());
    connect(actionExportVSFSToGodot, SIGNAL(triggered()), this, SLOT(exportShadersVSFSToGodot()));

    QAction *actionOpen = new QAction("Open shaders...", &g);
    connect(actionOpen, SIGNAL(triggered()), this, SLOT(attachShaders()));

    QAction *actionVS = new QAction("Vertex shader", &g);
    actionVS->setCheckable(true);
    actionVS->setChecked(enabled_vs);
    connect(actionVS, SIGNAL(toggled(bool)), this, SLOT(switchStatusVS()));

    QAction *actionGS = new QAction("Geometry shader", &g);
    actionGS->setCheckable(true);
    actionGS->setChecked(enabled_gs);
    connect(actionGS, SIGNAL(toggled(bool)), this, SLOT(switchStatusGS()));

    QAction *actionFS = new QAction("Fragment shader", &g);
    actionFS->setCheckable(true);
    actionFS->setChecked(enabled_fs);
    connect(actionFS, SIGNAL(toggled(bool)), this, SLOT(switchStatusFS()));

    QAction *actionRunTest = new QAction("Run test", &g);
    connect(actionRunTest, SIGNAL(triggered()), this, SLOT(compareTest()));

    // Scene
    QAction *actionAddObject = new QAction("Add object...", &g);
    connect(actionAddObject, SIGNAL(triggered()), &g, SLOT(addObject()));

    QAction *actionLoadObject = new QAction("Load object...", &g);
    connect(actionLoadObject, SIGNAL(triggered()), this, SLOT(loadObject()));

    QAction *actionClearScene = new QAction("Clear scene", &g);
    connect(actionClearScene, SIGNAL(triggered()), &g, SLOT(clearScene()));

    // Render options
    QAction *actionWireframe = new QAction("Wireframe rendering", &g);
    ...
    QAction *actionShowAxes = new QAction("Show Axes", &g);
    ...
    QAction *actionShowLight = new QAction("Show Light", &g);
    ...
    // Material
    QAction *actionMaterialAmbient = new QAction("Set ambient color...", &g);
    connect(actionMaterialAmbient, SIGNAL(triggered()), this, SLOT(setMaterialAmbient()));

    QAction *actionMaterialDiffuse = new QAction("Set diffuse color...", &g);
    connect(actionMaterialDiffuse, SIGNAL(triggered()), this, SLOT(setMaterialDiffuse()));

    QAction *actionMaterialSpecular = new QAction("Set specular color...", &g);
    ...
    if (program && program->isLinked())
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            if (type == GL_BOOL)
            {
                tmp = new QAction(QString("") + typeName + " " + QString(name) + QString(" = ") + value + QString(" [switch]"), &g);
            }
            else
            {
                if (type == GL_FLOAT || type == GL_INT)
                    tmp = new QAction(QString("") + typeName + " " + QString(name) + QString(" = ") + value + QString(" [edit...]"), &g);
                else
                    tmp = new QAction(QString("") + typeName + " " + QString(name) + QString(" = ") + value + QString(""), &g);
            }
    ...
    if (program && program->isLinked())
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            tmp = new QAction(QString("") + typeName + " " + QString(name) + QString(" = ") + value, &g);
            tmp->setData(QVariant(0)); // uniform id
    ...
    shaders->addAction(actionNewVSFSInGodot);
    shaders->addAction(actionExportVSFSToGodot);
...
void ShaderLoader::editUniform(int id)
{
    ...
    if (type == GL_FLOAT)
    {
        ...
        float value = fparams[0];
        bool active = true;
        CustomDialog d("Edit uniform " + QString(name), &g);
    ...
    {
        g.glGetUniformiv(program->programId(), id, iparams);
        bool value = not iparams[0];
        bool active = true;
        CustomDialog d("Edit uniform " + QString(name), &g);
        d.addLabel("Toggle \"" + QString(name) + QString("\""));
        d.addCheckBox("Active uniform (toggle with UP/DOWN keys)", &active);
        d.addCheckBox("Value: ", &value);
        d.exec();
        g.makeCurrent();
...
#else
    QImage im0 = img1.convertToFormat(QImage::Format_ARGB32).rgbSwapped().flipped(Qt::Vertical);
#endif
    g.makeCurrent();
...
void ShaderLoader::reloadShaders()
{
    ...
    // After all is said and done, report compilation errors, if any:
    if (enabled_vs and !vs->isCompiled() and not testMode)
    {
        QMessageBox::warning(glwidget(), "ShaderLoader",
                             QString("The Vertex Shader did not compile. \n See log for errors.\n") + vs->log(), QMessageBox::Ok);
        glwidget()->makeCurrent();
    }
    if (enabled_gs and !gs->isCompiled() and not testMode)
    {
        QMessageBox::warning(glwidget(), "ShaderLoader",
    ...
    if (enabled_fs and !fs->isCompiled() and not testMode)
    {
        QMessageBox::warning(glwidget(), "ShaderLoader",
    ...
    if (enabled_vs || enabled_gs || enabled_fs)
    {
        ...
        if ((!enabled_vs || vs->isCompiled()) && (!enabled_fs || fs->isCompiled()) && (!enabled_gs || gs->isCompiled()))
        {
            ...
            if (program->log().size())
                qDebug() << program->log() << "\n";
            if (!program->isLinked() and not testMode)
            {
                QMessageBox::warning(glwidget(), "ShaderLoader",
...
void ShaderLoader::checkForUpdates()
{
    ...
    if (enabled_vs)
    {
        ...
        if (info.lastModified() != lastModifiedVS)
            reload = true;
    }
    if (enabled_gs)
    {
        QFileInfo info(filename + ".geom");
        if (info.lastModified() != lastModifiedGS)
            reload = true;
    }
    if (enabled_fs)
    {
        QFileInfo info(filename + ".frag");
        if (info.lastModified() != lastModifiedFS)
            reload = true;
    }
    if (reload)
        reloadShaders();
...
void ShaderLoader::exportShadersVSFSToGodot()
{
    QString name = QFileDialog::getOpenFileName(glwidget(),
        tr("Export Shader"), QDir::currentPath(), tr("Shader Files (*.vert *.frag)"));
    if (name != "")
    {
        name.chop(5);
        attachShaders(name);
        GdShaderTrans translator;
        translator.exportToGodot(name);

        //Abrir Godot y ejecutar gdscript para abrir escena con objeto y shader exportado
        cout << "Opening Godot" << endl;
        char* tmp = getenv("VTEXTURES"); 
        string cmd;
        if (tmp != NULL) 
        { 
            cmd = "godot4 --headless --editor --path " + string(tmp) + 
                "/.. --script godot/load_exported_shader.gd --quit"; 
        }

        int result = system(cmd.c_str());
        if (result != 0) { std::cerr << "Error ejecutando script Godot\n"; return; }

        result = system(("godot4 --rendering-driver opengl3 -e " + string(tmp) +
            "/../godot/DefaultScene.tscn").c_str());
...
void ShaderLoader::drawHelp()
{
    ...
    // path
    // g.renderText(x,y, QString("Current path is ") + QDir::currentPath() + QString(" - Press SPACE for menu options"));
    // y+=18;
...
bool ShaderLoader::paintGL()
{
    ...
    if (lightRotate)
    {
        ...
        float z = 4 * sin(4 * t);
        lightPosition = QVector4D(scene()->boundingBox().center(), 1.) + QVector4D(r * x, r * y, r * z, 0.);
    ...
    if (program && program->isLinked())
    {
        ...
        QPoint p = g.mapFromGlobal(QCursor::pos());
        program->setUniformValue("mousePosition", QVector2D(float(p.x()), g.height() - float(p.y())));
        predefinedUniforms.push_back("mousePosition");

        int width = g.width();
        int height = g.height();
        program->setUniformValue("viewport", QVector2D(width, height));
        predefinedUniforms.push_back("viewport");
        ...
        if (testMode)
        {
            ...
            for (unsigned int i = 0; i < testUniformvec2Names.size(); ++i)
                program->setUniformValue(testUniformvec2Names[i].toStdString().c_str(), testUniformvec2Values[i]);
        }
...
void ShaderLoader::createDefaultShadersVSFSGodot()
{
    char* tmp = getenv("VTEXTURES");
    string cmd;
    bool scene = false; //indicates if the godot scene was created
    if (tmp != NULL)
    {
        cmd =
            "godot4 --headless --editor --path " + string(tmp) + "/.. --script godot/create_scene_runtime.gd --quit";
        scene = true;
    }

    int result = system(cmd.c_str());
    cout << "Abriendo Godot" << endl;
    if (result != 0)
        std::cerr << "Error ejecutando Godot\n";
    else if(scene)
    {
        result = system("godot4 --rendering-driver opengl3 -e godot/DefaultScene.tscn");
    }
}
...
void ShaderLoader::createDefaultShaders(bool createAlsoGS)
{
    ...
    if (true)
    {
        ...
        out << "#version 330 core" << "\n"
            << "\n";
        out << "layout (location = 0) in vec3 vertex;" << "\n";
        out << "layout (location = 1) in vec3 normal;" << "\n";
        out << "layout (location = 2) in vec3 color;" << "\n";
        out << "layout (location = 3) in vec2 texCoord;" << "\n"
            << "\n";

        if (!createAlsoGS)
        {
            out << "out vec4 frontColor;" << "\n";
            out << "out vec2 vtexCoord;" << "\n"
                << "\n";
        }
        else
            out << "out vec4 vfrontColor;" << "\n"
                << "\n";

        out << "uniform mat4 modelViewProjectionMatrix;" << "\n";
        out << "uniform mat3 normalMatrix;" << "\n"
            << "\n";
        out << "void main()" << "\n";
        out << "{" << "\n";
        out << "    vec3 N = normalize(normalMatrix * normal);" << "\n";
        if (!createAlsoGS)
        {
            out << "    frontColor = vec4(color,1.0) * N.z;" << "\n";
            out << "    vtexCoord = texCoord;" << "\n";
        }
        else
            out << "    vfrontColor = vec4(color,1.0) * N.z;" << "\n";
        out << "    gl_Position = modelViewProjectionMatrix * vec4(vertex, 1.0);" << "\n";
        out << "}" << "\n";
    ...
    if (createAlsoGS)
    {
        ...
        out << "layout(triangle_strip, max_vertices = 36) out;" << "\n";
        out << "" << "\n";
        out << "in vec4 vfrontColor[];" << "\n";
        out << "out vec4 gfrontColor;" << "\n";
        out << "" << "\n";
        out << "void main( void )" << "\n";
        out << "{" << "\n";
        out << "	for( int i = 0 ; i < 3 ; i++ )" << "\n";
        out << "	{" << "\n";
        out << "		gfrontColor = vfrontColor[i];" << "\n";
        out << "		gl_Position = gl_in[i].gl_Position;" << "\n";
        out << "		EmitVertex();" << "\n";
        out << "	}" << "\n";
        out << "    EndPrimitive();" << "\n";
        out << "}" << "\n";
    ...
    if (true)
    {
        ...
        out << "" << "\n";
        if (!createAlsoGS)
            out << "in vec4 frontColor;" << "\n";
        else
            out << "in vec4 gfrontColor;" << "\n";
        out << "out vec4 fragColor;" << "\n";
        out << "" << "\n";
        out << "void main()" << "\n";
        out << "{" << "\n";
        if (!createAlsoGS)
            out << "    fragColor = frontColor;" << "\n";
        else
            out << "    fragColor = gfrontColor;" << "\n";
        out << "}" << "\n";
        fileFS.close();
    }
...
QString ShaderLoader::executeTest() {
    ...
    if (!QFile::exists(testFilename))
    {
        ...
        QFileInfo fi(filename + ".vertgrab");
    ...
    while (!f.eof())
    {
        ...
        {
            f >> s;
            bool saveShowLight = showLight;
            showLight = false;
            QOpenGLFramebufferObject fbo(g.size(), fboFormat);
            fbo.bind();
            g.glViewport(0, 0, g.width(), g.height());
            g.paintGL();
            showLight = saveShowLight;
        ...
        else if (cmd == "vec2")
        {
            float a, b;
            f >> s >> a >> b;
            testUniformvec2Names.push_back(s.c_str());
            testUniformvec2Values.push_back(QVector2D(a, b));
        }
        else if (cmd == "time")
        {
            float value;
